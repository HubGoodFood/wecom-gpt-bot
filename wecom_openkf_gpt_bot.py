...
def wechat_kf_callback():
    ...
    decrypted = crypto.decrypt_message(raw_msg, msg_signature, timestamp, nonce)
    if not decrypted.strip():
        logging.error("❌ 解密失败，收到空消息")
        return "ok"

    if decrypted.strip().startswith("<xml>"):
        # 是 XML 类型消息（如 kf_msg_or_event）
        try:
            root = ET.fromstring(decrypted)
            token = root.findtext("Token")
            open_kfid = root.findtext("OpenKfId")
            logging.info("📥 解密后 XML: %s", decrypted)
        ...
        try:
            ...
            logging.info("📥 解密后 JSON: %s", msg_json)
        except json.JSONDecodeError:
            logging.error("❌ JSON 解码失败: 内容为 %s", decrypted)
            return "ok"
        return "ok"
