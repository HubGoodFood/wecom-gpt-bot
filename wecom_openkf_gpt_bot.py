...
def is_duplicate(user_id, content):
    if user_id not in last_user_messages:
        ...
        return False
    # 如包含关键疑问词，强制认为不是重复
    if any(q in content for q in ["什么", "怎么买", "如何", "几块", "多少", "有没有"]):
        return False
    if content in last_user_messages[user_id]:
        return True
    return False

def remember_message(user_id, content):
    if user_id not in last_user_messages:
        last_user_messages[user_id] = deque(maxlen=5)
    last_user_messages[user_id].append(content)
    ...
    if request.method == "POST":
        try:
            ...
            if is_duplicate(user_id, content):
                ...
                return "ok", 200
            remember_message(user_id, content)
