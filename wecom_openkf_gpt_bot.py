...
def extract_token_from_xml(xml_str):
    try:
        root = ET.fromstring(xml_str)
        return root.find("Token").text
    except Exception as e:
        logging.warning("未找到 Token: %s", str(e))
        return ""
...
def send_wechat_message(userid, content):
    ...
    logging.info("📤 微信发送结果: %s", r.json())

def pull_latest_text_message(open_kfid, token):
    access_token = get_wechat_access_token()
    ...
    if request.method == "POST":
        try:
            ...
            if "kf_msg_or_event" in decrypted_xml:
                token = extract_token_from_xml(decrypted_xml)
                userid, text = pull_latest_text_message(OPEN_KFID, token)
                if userid and text:
                    logging.info("💬 用户 [%s] 发来: %s", userid, text)
                    reply = query_product_price(text) or query_with_gpt(text)
                    send_wechat_message(userid, reply)
            return "success", 200
        except Exception as e:
