    ...
    while True:
        ...
        for msg in data.get("msg_list", []):
            if msg["msgtype"] == "text" and msg["open_kfid"] == open_kfid:
                return msg["external_userid"], msg["text"]["content"], msg["msgid"]
...
2. 遇到模糊提问（如“你们买什么”、“怎么买”）要主动介绍商品和服务。
...
def callback():
    ...
    if request.method == "POST":
        try:
            ...
            user_id, content, msgid = pull_latest_text(OPEN_KFID, token)

            if not user_id or not content:
                return "ok", 200

            # 去重：如果是重复消息就不发
            if last_messages.get(user_id) == msgid:
                logging.info("⚠️ 忽略重复消息: %s", msgid)
                return "ok", 200
            last_messages[user_id] = msgid
