...
def wechat_callback():
    ...
    try:
        ...
        if request.method == "POST":
            ...
            to_user = msg.get("FromUserName")    # 正确：要回给谁
            from_user = msg.get("ToUserName")    # 正确：机器人是谁

            logging.info("🧾 用户发来内容: %s", content)

            reply = "你好，我是果蔬客服机器人，正在测试中~"

            reply_xml = f"<xml><ToUserName><![CDATA[{to_user}]]></ToUserName><FromUserName><![CDATA[{from_user}]]></FromUserName><CreateTime>{int(time.time())}</CreateTime><MsgType><![CDATA[text]]></MsgType><Content><![CDATA[{reply}]]></Content></xml>"
